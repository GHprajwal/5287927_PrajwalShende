...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    int lastAnswer = 0;
    int *ans = malloc(queries_rows * sizeof(int)); // max possible
    int ansCount = 0;
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (type == 1) {
            ...
            if (seqSizes[idx] == seqCapacity[idx]) {
                seqCapacity[idx] = seqCapacity[idx] ? seqCapacity[idx] * 2 : 2;
    ...
    *result_count = ansCount;
    ans = realloc(ans, ansCount * sizeof(int)); // trim to exact size
