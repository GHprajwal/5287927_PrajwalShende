...
char** fillGrid(int rows, int cols) {
...
char** bomberMan(int n, int r, char** grid, int* result_count) {
    ...
    *result_count = r;
    ...
    if (n == 1) return grid;
    if (n % 2 == 0) return fillGrid(r, c);

    char **firstBlast = detonate(r, c, grid);
    if (n % 4 == 3) return firstBlast;

    char **secondBlast = detonate(r, c, firstBlast);
    return secondBlast;
}
