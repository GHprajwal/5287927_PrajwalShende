...
char* highestValuePalindrome(char* s, int n, int k) {
    ...
    for (int i = 0; i < n / 2; i++) {
        if (result[i] != result[n - 1 - i]) {
            char bigger = result[i] > result[n - 1 - i] ? result[i] : result[n - 1 - i];
            result[i] = result[n - 1 - i] = bigger;
            changed[i] = 1;
            used++;
        }
    }
    ...
    for (int i = 0; i < n / 2 && used < k; i++) {
        if (result[i] != '9') {
            if (changed[i] && used + 1 <= k) {
                result[i] = result[n - 1 - i] = '9';
                used++;
            } else if (!changed[i] && used + 2 <= k) {
                result[i] = result[n - 1 - i] = '9';
                used += 2;
            }
        }
