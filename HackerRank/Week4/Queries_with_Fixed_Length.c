...
int* solve(int n, int* arr, int q, int* queries, int* result_count) {
    int* span_left = malloc(n * sizeof(int));
    int* span_right = malloc(n * sizeof(int));
    ...
    for (int i = 0; i < n; i++) {
        ...
        span_left[i] = (top == -1) ? (i + 1) : (i - stack[top]);
        stack[++top] = i;
    }
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        span_right[i] = (top == -1) ? (n - i) : (stack[top] - i);
        stack[++top] = i;
    }
    ...
    for (int i = 0; i < n; i++) {
        int length = span_left[i] + span_right[i] - 1;
        if (arr[i] < best[length]) {
            best[length] = arr[i];
        }
    }
